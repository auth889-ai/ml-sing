...
| flash-attn wheel | **NOT YET PRODUCED.** Attempt #1 (pip isolated build) linked against wrong torch → `ImportError: undefined symbol _ZNK3c104cuda10CUDAStream5queryEv`; bad wheel PURGED from Drive and pip caches. Attempt #3 RUNNING: `pip wheel flash-attn==2.8.3.post1 --no-build-isolation --no-deps --no-cache-dir -w /content/wheels_out`, MAX_JOBS=4, TORCH_CUDA_ARCH_LIST=8.9, inside the Colab **tmux terminal session** (job of the tmux bash, survives socket drops; do NOT close the terminal PANE — closing the pane killed a previous attempt). Last confirmed: **67 objects compiled, 2 nvcc active, ~1 obj/min** |
